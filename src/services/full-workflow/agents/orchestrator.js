...
export async function runAgentPipeline(workflowId, userId, options = {}) {
  ...
  try {
    ...
    logger.warn('Orchestrator: workflow not found or unauthorized', { workflowId, userId, error: err.message });
  ...
  try {
    ...
    logger.warn('Orchestrator: failed to load stats/history', { workflowId, error: err.message });
  ...
  if (!options.skipMonitoring) {
    ...
    try {
      ...
      logger.error('Orchestrator: monitoring agent failed', { workflowId, error: err.message });
  ...
  const securityPromise = !options.skipSecurity
    ? (async () => {
        try {
          ...
          logger.error('Orchestrator: security agent failed', { workflowId, error: err.message });
  ...
  const executionPromise = !options.skipExecution
    ? (async () => {
        try {
          ...
          logger.error('Orchestrator: execution agent failed', { workflowId, error: err.message });
          return { success: false, error: err.message };
        }
      })()
    : Promise.resolve(null);
  ...
  [results.security, results.execution] = await Promise.all([securityPromise, executionPromise]);
  ...
  if (!options.skipOptimization) {
    ...
    try {
      ...
      logger.error('Orchestrator: optimization agent failed', { workflowId, error: err.message });
  ...
  await logAgentAction({
    ...
    optimizationImpact: results.autoApplyResult?.appliedCount > 0 ? 'helped' : 'neutral',
  });
